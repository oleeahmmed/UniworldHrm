...
class Employee(models.Model):
    """Enhanced Employee model with comprehensive personal and employment details."""
    ...
    # Removed DIVISION_CHOICES and DISTRICT_CHOICES from here as they will be loaded dynamically
    # Bangladeshi Division and District fields will now be CharFields without choices initially.
    ...
    name = models.CharField(_("Full Name"), max_length=200, null=True, blank=True)
    ...
    spouse_name = models.CharField(_("Spouse Name"), max_length=100, null=True, blank=True)
    
    # Identity Information
    card_no = models.CharField(_("Card No"), max_length=20, unique=True, null=True, blank=True)
    national_id = models.CharField(_("National ID"), max_length=50, unique=True, null=True, blank=True)
    passport_number = models.CharField(_("Passport Number"), max_length=50, null=True, blank=True)
    driving_license = models.CharField(_("Driving License"), max_length=50, null=True, blank=True)
    tax_id = models.CharField(_("Tax ID"), max_length=50, null=True, blank=True)
    social_security_number = models.CharField(_("Social Security Number"), null=True, blank=True, max_length=50)
    
    # Personal Details
    gender = models.CharField(_("Gender"), max_length=1, choices=GENDER_CHOICES)
    date_of_birth = models.DateField(_("Date of Birth"))
    place_of_birth = models.CharField(_("Place of Birth"), max_length=100, null=True, blank=True)
    nationality = models.CharField(_("Nationality"), max_length=100, null=True, blank=True)
    religion = models.CharField(_("Religion"), max_length=50, choices=RELIGION_CHOICES, null=True, blank=True)
    blood_group = models.CharField(_("Blood Group"), max_length=3, choices=BLOOD_GROUP_CHOICES, null=True, blank=True)
    marital_status = models.CharField(_("Marital Status"), max_length=2, choices=MARITAL_STATUS_CHOICES)
    ...
    emergency_contact_relationship = models.CharField(_("Emergency Contact Relationship"), max_length=50, null=True, blank=True)
    emergency_contact_address = models.TextField(_("Emergency Contact Address"), null=True, blank=True)
    
    # Secondary Emergency Contact
    emergency_contact_2_name = models.CharField(_("Emergency Contact 2 Name"), max_length=100, null=True, blank=True)
    emergency_contact_2_phone = models.CharField(_("Emergency Contact 2 Phone"), max_length=20, null=True, blank=True)
    emergency_contact_2_relationship = models.CharField(_("Emergency Contact 2 Relationship"), max_length=50, null=True, blank=True)
    
    # Present Address (Bangladeshi Style)
    present_village_house = models.CharField(_("Present Village/House No."), max_length=255, null=True, blank=True)
    present_road_block = models.CharField(_("Present Road/Block/Sector"), max_length=255, null=True, blank=True)
    present_post_office = models.CharField(_("Present Post Office"), max_length=100, null=True, blank=True)
    present_police_station = models.CharField(_("Present Police Station (PS)"), max_length=100, null=True, blank=True)
    # Changed to CharField without choices for dynamic loading
    present_district = models.CharField(_("Present District"), max_length=100, null=True, blank=True)
    present_division = models.CharField(_("Present Division"), max_length=100, null=True, blank=True)
    present_postal_code_bd = models.CharField(_("Present Postal Code"), max_length=10, null=True, blank=True)
    
    # Permanent Address (Bangladeshi Style)
    permanent_village_house = models.CharField(_("Permanent Village/House No."), max_length=255, null=True, blank=True)
    permanent_road_block = models.CharField(_("Permanent Road/Block/Sector"), max_length=255, null=True, blank=True)
    permanent_post_office = models.CharField(_("Permanent Post Office"), max_length=100, null=True, blank=True)
    permanent_police_station = models.CharField(_("Permanent Police Station (PS)"), max_length=100, null=True, blank=True)
    # Changed to CharField without choices for dynamic loading
    permanent_district = models.CharField(_("Permanent District"), max_length=100, null=True, blank=True)
    permanent_division = models.CharField(_("Permanent Division"), max_length=100, null=True, blank=True)
    permanent_postal_code_bd = models.CharField(_("Permanent Postal Code"), max_length=10, null=True, blank=True)

    # Mailing Address (can be a copy of present or permanent, or separate)
    mailing_address = models.TextField(_("Mailing Address"), null=True, blank=True)
    ...
    employment_status = models.CharField(_("Employment Status"), max_length=20, 
                                         choices=EMPLOYMENT_STATUS_CHOICES, default='PERMANENT')
    employment_type = models.CharField(_("Employment Type"), max_length=20, 
                                       choices=EMPLOYMENT_TYPE_CHOICES, default='FULL_TIME')
    employee_grade = models.CharField(_("Employee Grade"), max_length=50, null=True, blank=True)
    ...
    reporting_manager = models.ForeignKey('self', on_delete=models.SET_NULL, null=True, blank=True,
                                         related_name='subordinates', verbose_name=_("Reporting Manager"))
    
    # Work Configuration
    expected_work_hours = models.PositiveIntegerField(_("Expected Work Hours"), default=8) 
    overtime_grace_minutes = models.PositiveIntegerField(_("Overtime Grace Minutes"), default=15)
    work_location = models.CharField(_("Work Location"), max_length=200, null=True, blank=True)
    ...
    currency = models.CharField(_("Currency"), max_length=10, default='BDT', null=True, blank=True) # Added currency field

    # Bank Information
    bank_name = models.CharField(_("Bank Name"), max_length=100, null=True, blank=True)
    bank_branch = models.CharField(_("Bank Branch"), max_length=100, null=True, blank=True)
    bank_account_number = models.CharField(_("Bank Account Number"), max_length=50, null=True, blank=True)
    bank_routing_number = models.CharField(_("Bank Routing Number"), max_length=50, null=True, blank=True)
    bank_swift_code = models.CharField(_("Bank SWIFT Code"), max_length=20, null=True, blank=True)
    
    # Additional Information
    languages_spoken = models.TextField(_("Languages Spoken"), null=True, blank=True,
    ...
    termination_reason = models.TextField(_("Termination Reason"), null=True, blank=True)
    
    # Profile and Documents
    profile_picture = models.ImageField(_("Profile Picture"), upload_to='employee_profiles/', 
                                         null=True, blank=True)
    signature = models.ImageField(_("Signature"), upload_to='employee_signatures/', 
                                 null=True, blank=True)
    ...
    created_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True,
                                  related_name='created_employees', verbose_name=_("Created By"))
    updated_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True,
                                  related_name='updated_employees', verbose_name=_("Updated By"))
    ...
    @property
    def age(self):
        if self.date_of_birth: # Added a check for date_of_birth
    ...
    def service_duration(self):
        # This property might need adjustment based on your 'separation' model if it exists
        # For simplicity, assuming it's still relevant or can be adapted.
    ...
    def save(self, *args, **kwargs):
        ...
        # Set mailing address to present address if not provided
        # This logic needs to be updated to use the new Bangladeshi address fields
        if not self.mailing_address:
            # Construct mailing_address from present address components
            present_address_parts = [
                self.present_village_house,
                self.present_road_block,
                self.present_post_office,
                self.present_police_station,
                self.present_district,
                self.present_division,
                self.present_postal_code_bd,
            ]
            # Filter out None and empty strings before joining
            self.mailing_address = ", ".join(filter(None, present_address_parts))
...
class EducationLevel(models.Model):
    """Defines education levels/degrees."""
    name = models.CharField(_("Name"), max_length=100)
    code = models.CharField(_("Code"), max_length=20, unique=True)
    level = models.PositiveIntegerField(_("Level"), help_text="Numeric level for ordering (1=Primary, 2=Secondary, etc.)")
...
class EducationQualification(models.Model):
    """Employee education qualifications (simplified)."""
    
    employee = models.ForeignKey(Employee, on_delete=models.CASCADE, 
                               related_name='education_qualifications', verbose_name=_("Employee"))
    education_level = models.ForeignKey(EducationLevel, on_delete=models.CASCADE,
                                      related_name='qualifications', verbose_name=_("Education Level"))
    institution_name = models.CharField(_("Institution Name"), max_length=200)
    degree_title = models.CharField(_("Degree/Certificate Title"), max_length=200)
    ...
    def __str__(self):
        return f"{self.employee.get_full_name()} - {self.degree_title} from {self.institution_name}"
...
class JobExperience(models.Model):
    """Employee previous job experience (simplified)."""
    
    # You might still need these choices if you plan to re-introduce the field later
    # or if other parts of your application still reference them.
    EMPLOYMENT_TYPE_CHOICES = (
        ('FULL_TIME', 'Full Time'),
        ('PART_TIME', 'Part Time'),
        ('CONTRACT', 'Contract'),
    ...
    employee = models.ForeignKey('Employee', on_delete=models.CASCADE, 
                               related_name='job_experiences', verbose_name=_("Employee"))
    ...
    # Make employment_type nullable to resolve the IntegrityError
    # Even if you don't use it in the form, the database expects it.
    employment_type = models.CharField(_("Employment Type"), max_length=20, 
                                     choices=EMPLOYMENT_TYPE_CHOICES, null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.employee.get_full_name()} - {self.job_title} at {self.company_name}"
...
class DocumentType(models.Model):
    """Defines types of documents that can be stored for employees (simplified)."""
    name = models.CharField(_("Name"), max_length=100)
    code = models.CharField(_("Code"), max_length=20, unique=True)
    description = models.TextField(_("Description"), null=True, blank=True)
    created_at = models.DateTimeField(_("Created At"), auto_now_add=True)
...
class EmployeeDocument(models.Model):
    """Stores various documents for employees (simplified)."""
    STATUS_CHOICES = (
        ('PENDING', 'Pending'),
        ('APPROVED', 'Approved'),
        ('REJECTED', 'Rejected'),
    ...
    document_type = models.ForeignKey(DocumentType, on_delete=models.CASCADE,
                                    related_name='employee_documents', verbose_name=_("Document Type"))
    
    # Document Information
    document_name = models.CharField(_("Document Name"), max_length=200)
    
    # File and Status
    document_file = models.FileField(_("Document File"), upload_to='employee_documents/')
    ...
    def __str__(self):
        return f"{self.employee.get_full_name()} - {self.document_name}"
    
    def save(self, *args, **kwargs):
        # Removed auto-update status based on expiry as expiry_date is removed
        super().save(*args, **kwargs)
...
class FamilyMember(models.Model):
    """Employee family member information (simplified)."""
    RELATIONSHIP_CHOICES = (
        ...
        ('BROTHER', 'Brother'),
        ('SISTER', 'Sister'),
        ('OTHER', 'Other'), # Simplified choices
    )
    
    employee = models.ForeignKey('Employee', on_delete=models.CASCADE, 
                               related_name='family_members', verbose_name=_("Employee"))
    ...
    # System Fields (keeping for basic tracking, but not exposed in simplified form)
    created_at = models.DateTimeField(_("Created At"), auto_now_add=True)
    updated_at = models.DateTimeField(_("Updated At"), auto_now=True)
    
    def __str__(self):
        return f"{self.employee.get_full_name()} - {self.name} ({self.get_relationship_display()})"
    
    # Removed age property as it's not essential for a simplified model
